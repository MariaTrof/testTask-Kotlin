...
describe('WebSocketClient', () => {
    ...
    beforeEach(() => {
        global['WebSocket'] = function WebSocket(url, protocol) {
            ...
            this.send = mock_functions_1.createMockFunction();
            socketMock = this;
        };
    });
    afterEach(() => {
    ...
    it('should connect to the passed url', () => {
        new client_1.WebSocketClient({ url: 'ws://test' });
    ...
    it('should call onOpen if provided', () => {
        const onOpen = mock_functions_1.createMockFunction();
        new client_1.WebSocketClient({ url: 'ws://test', onOpen: onOpen });
        socketMock.onopen();
    ...
    it('should call onopen on protocols', () => {
        ...
        socket.registerProtocol('test', { onopen: onopen, onmessage: mock_functions_1.createMockFunction() });
        socketMock.onopen();
        chai_1.expect(onopen.calls.length).to.equal(1);
    });
...
describe('clientMiddleware', () => {
    ...
    it('should send dispatched actions with meta.toServer to the socket', () => {
        const socket = socket_1.createMockSocket();
        const dispatch = client_1.websocketMiddleware({ socket: socket })(null)(mock_functions_1.createMockFunction());
    ...
    it('should send dispatched actions with meta.toServer in actions to the socket', () => {
        ...
        const dispatch = client_1.websocketMiddleware({ socket: socket, actions: actions })(null)(mock_functions_1.createMockFunction());
        const sendMock = socket.protocols['action'].send = mock_functions_1.createMockFunction();
        dispatch({ type: 'dispatched' });
        chai_1.expect(sendMock.calls.length).to.equal(1);
        const firstCall = sendMock.calls[0];
